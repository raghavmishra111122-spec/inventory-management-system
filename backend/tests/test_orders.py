def test_create_order_success(client):
    ...
    p1_get = client.get(f"/products/{prod1_id}").json()
    p2_get = client.get(f"/products/{prod2_id}").json()
    assert p1_get["quantity_in_stock"] == 8
    assert p2_get["quantity_in_stock"] == 40


def test_insufficient_stock_fails(client):
    cust_res = client.post("/customers", json={
        "full_name": "Alice Mercer",
        "email": "alice@gmail.com",
        "phone_number": "555-1234"
    ...
    prod_get = client.get(f"/products/{prod_id}").json()
    assert prod_get["quantity_in_stock"] == 5


def test_transactional_rollback_on_partial_failure(client):
    cust_res = client.post("/customers", json={
        "full_name": "Alice Mercer",
        "email": "alice@gmail.com",
        "phone_number": "555-1234"
    ...
    prodA_res = client.post("/products", json={
    ...
    prodB_res = client.post("/products", json={
    ...
    pA_get = client.get(f"/products/{prodA_id}").json()
    pB_get = client.get(f"/products/{prodB_id}").json()
...
def test_delete_order_restores_stock(client):
    ...
    p_after_order = client.get(f"/products/{prod_id}").json()
    ...
    p_after_delete = client.get(f"/products/{prod_id}").json()
